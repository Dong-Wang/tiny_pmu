...
{	
	...
	//calculte base event
	retire_uop_fraction = IA32_SCALE_FACTOR * slots_retired / uops_issues_any;
	ms_slots_retired = retire_uop_fraction * ms_uops / IA32_SCALE_FACTOR;
	ops_executed = cycle_activity_stalls_total - rs_empty_cycles + cycles_ge_1_uop_exec - cycles_ge_2_uop_exec;
	l3_hit_fraction = IA32_SCALE_FACTOR * uops_retired_llc_hit / (uops_retired_llc_hit + uops_retired_llc_miss);
	mem_stalls_l3miss = (IA32_SCALE_FACTOR - l3_hit_fraction) * memstall_l2miss;
	
	//calculate category level 1
	frontend_bound=	IA32_SCALE_FACTOR * IA32_THREAD_PER_CORE * fetch_bubbles / total_slots;
	bad_speculation = IA32_SCALE_FACTOR * (slots_issued-slots_retired + (4 * recovery_bubbles)/IA32_THREAD_PER_CORE) / (total_slots / IA32_THREAD_PER_CORE);
	retiring = IA32_SCALE_FACTOR * slots_retired / (total_slots / IA32_THREAD_PER_CORE);
	bankend_bound = IA32_SCALE_FACTOR-(frontend_bound + bad_speculation + retiring);
	
	//calculate category level 2
	fetch_latency_bound = IA32_SCALE_FACTOR * fetch_bubbles_MIW / ( clock / IA32_THREAD_PER_CORE );
	fetch_bandwidth_bound = IA32_SCALE_FACTOR * (fetch_bubbles - 4 * fetch_bubbles_MIW)/ ( total_slots / IA32_THREAD_PER_CORE );
	br_mispred_fraction = IA32_SCALE_FACTOR * br_mispred_retired / (br_mispred_retired + machine_clears_count);
	branch_mispredicts = bad_speculation * br_mispred_fraction / IA32_SCALE_FACTOR;
	machine_clear = bad_speculation - branch_mispredicts;
	micro_sequencer = IA32_SCALE_FACTOR * ms_uops / total_slots;
	base = retiring - micro_sequencer;
    
	memory_bound = memory_bound_formula(IA32_SCALE_FACTOR,IA32_THREAD_PER_CORE,
		fetch_bubbles,slots_issued,recovery_bubbles,clock,slots_retired,
		memstall_anyload,exe_activity_bound_on_stores,exe_activity_1_ports_util,
		exe_activity_2_ports_util,exe_activity_exe_bound_0_ports,clock,slots_issued);
	core_bound = core_bound_formula(IA32_SCALE_FACTOR,IA32_THREAD_PER_CORE,
		fetch_bubbles,slots_issued,recovery_bubbles,clock,slots_retired,
		memstall_anyload,exe_activity_bound_on_stores,exe_activity_1_ports_util,
		exe_activity_2_ports_util,exe_activity_exe_bound_0_ports,clock,slots_issued);
